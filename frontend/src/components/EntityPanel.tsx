      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <div className="p-3.5 rounded-xl bg-slate-900/60 border border-slate-800">
          ...
          {entities.recruiter_email && (entities.recruiter_email.includes('gmail.com') || entities.recruiter_email.includes('outlook.com')) && (
            <span className="inline-block mt-1 text-[10px] text-rose-400 font-medium">
              ⚠ Free Webmail Domain
            </span>
          )}
        </div>
        ...
        <div className="p-3.5 rounded-xl bg-slate-900/60 border border-slate-800">
          ...
          {entities.recruiter_name && (
            <p className="text-xs text-slate-400 mt-0.5">Name: {entities.recruiter_name}</p>
          )}
        </div>
          ...
          <div className="flex items-center gap-2 text-xs font-medium text-slate-400 mb-1">
            <AlertOctagon
              className={`w-3.5 h-3.5 ${
                entities.demanded_fee ? 'text-rose-400' : 'text-slate-400'
              }`}
            />
            <span className={entities.demanded_fee ? 'text-rose-400 font-bold' : ''}>
              Upfront Fee / Deposit
            </span>
          </div>
          <p
            className={`text-sm font-semibold truncate ${
              entities.demanded_fee ? 'text-rose-300' : 'text-slate-400'
            }`}
          >
      ...
      {entities.flags && entities.flags.length > 0 && (
        <div className="mt-4 pt-4 border-t border-slate-800/80 flex items-center gap-2 flex-wrap">
          ...
          {entities.flags.map((flag, idx) => (
            <span
              key={idx}
              className="text-[11px] px-2.5 py-0.5 rounded bg-rose-950/80 text-rose-300 border border-rose-800/60 font-mono"
            >
              {flag}
            </span>
          ))}
